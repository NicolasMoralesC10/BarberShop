...
fetch(base_url + "/dashboard/selectCitasWeek") // ajusta a tu endpoint real
  .then((response) => response.json())
  .then((data) => {
    // Si data no es un array, conviértelo en array con un solo elemento
    if (!Array.isArray(data)) {
      data = [data];
    }

    const dias = ["L", "M", "X", "J", "V", "S", "D"];
    const dataset = dias.map((d) => {
      const found = data.find((item) => item.dia === d);
